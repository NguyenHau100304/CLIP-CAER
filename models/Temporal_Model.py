...
class Temporal_Transformer_TAP(nn.Module):
    """
    Temporal Transformer với cơ chế Temporal Attention Pooling (TAP)
    Thay vì dùng CLS token, nó tự học trọng số để tổng hợp các frame.
    """
    def __init__(self, num_patches, input_dim, depth, heads, mlp_dim, dim_head):
        super().__init__()
        dropout = 0.1
        ...
        # Positional Embedding (Không cần +1 cho CLS token nữa)
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches, input_dim))
    ...
    def forward(self, x):  
        # x shape: (B, T, D) -> (8, 16, 512)
        b, n, _ = x.shape
        
        # 1. Cộng Positional Embedding
        x = x + self.pos_embedding[:, :n]
        
        # 2. Qua Transformer (giữ nguyên sequence)
        x = self.temporal_transformer(x) # (B, T, D)
        
        # 3. Temporal Attention Pooling
        # Tính trọng số alpha cho từng frame: (B, T, 1)
        alpha = self.attn_pool(x) 
        
        # Tổng có trọng số: sum(alpha * x) -> (B, D)
        # Broadcasting: (B, T, 1) * (B, T, D) -> (B, T, D) -> sum(dim=1) -> (B, D)
        x = (x * alpha).sum(dim=1)
